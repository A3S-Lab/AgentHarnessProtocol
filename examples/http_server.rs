                    ...
                    .and_then(|cmd| cmd.as_str())
                {
                    if command.contains("rm -rf") || command.contains("dd if=") {
                        return Ok(Decision::Block {
                            reason: "Dangerous command detected".to_string(),
                            metadata: None,
                        });
                    }
                }
...
async fn main() -> anyhow::Result<()> {
    ...
    // Create HTTP server
    let http_server = a3s_ahp::transport::http::HttpServer::new(server);

    println!("Starting HTTP server on http://0.0.0.0:8080/ahp");
    println!("Press Ctrl+C to stop");

    // Run server
    http_server.run(([0, 0, 0, 0], 8080)).await?;
