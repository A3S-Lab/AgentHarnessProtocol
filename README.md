...
AHP v2.2 introduces **driving points** — structured hooks that intercept agent operations at specific moments:

```
Agent Loop
    │
    ├─► PreContextPerception ──► [Block/Allow] ──► Model gets context
    ├─► PreMemoryRecall ────────► [Block/Allow] ──► Memory retrieved
    ├─► PrePlanning ───────────► [Block/Allow] ──► Task planned
    ├─► PreReasoning ──────────► [Block/Allow] ──► Reasoning started
    ├─► PreToolUse ────────────► [Block/Allow] ──► Tool executed
    ├─► PostToolUse ───────────► [Fire-and-forget]
    ├─► OnSuccess ─────────────► [Fire-and-forget]
    ├─► OnError ───────────────► [Fire-and-forget]
    ├─► OnRateLimit ───────────► [Fire-and-forget]
    └─► OnConfirmation ───────► [Block/Allow] ──► Human approved
```
...
For blocking events like `context_perception`, the harness can inject context into the agent:
...
```json
{
  ...
  "injected_context": {
    "facts": [
      { "content": "Validates credentials", "source": "auth.rs:42", "confidence": 0.95 }
    ],
    "file_contents": [
      { "path": "src/auth.rs", "snippet": "pub fn validate...", "relevance_score": 0.9 }
    ],
    "project_summary": {
